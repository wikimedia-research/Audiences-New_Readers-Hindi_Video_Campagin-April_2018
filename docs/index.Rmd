...
The estimated causal effect is 49.69K pageviews daily with a 95% interval of [-3.61K, 103.68K], which means this effect is not statistically significant and may be the result of random fluctuations that are unrelated to the intervention.

![](figures/hiwiki_external_pv_tv_impact.png)

### Unique Devices

For Hindi Wikipedia unique devices from India, the best model has a local level trend with prior standard deviation of the Gaussian random walk as 0.01 and a seasonality component, but no AR(1) or holiday state component. It uses a mixed of top 10 (among all control series and among all Hindi Wikipedia series) dynamic time warping-matched and correlated control series and the regression coefficients are static. The training data start from 2016-01-01 without log transformation. The estimated causal effect is 48.38K unique devices daily with a 95% interval of [-1.68K, 98.53K], which means this effect is not statistically significant and may be the result of random fluctuations that are unrelated to the intervention.

![](figures/hiwiki_ud_tv_impact.png)

For Hindi Wikipedia first visit unique devices from India, the best model has a local level trend with prior standard deviation of the Gaussian random walk as 0.01, AR(1) and holiday component, but no seasonality state component. It uses a mixed of top 30 (among all control series) dynamic time warping-matched and correlated control series and the regression coefficients are static. The training data start from 2017-10-01 without log transformation. The estimated causal effect is 25.39K unique devices daily with a 95% interval of [-3.56K, 48.23K], which means this effect is not statistically significant and may be the result of random fluctuations that are unrelated to the intervention.
