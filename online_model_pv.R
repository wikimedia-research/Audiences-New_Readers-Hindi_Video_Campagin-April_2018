...
# Try other top models to see if there is any difference
model_483 <- run_bsts_model(x, y, train_start, train_end, test_start, test_end, selected_controls,
  control_condidates = "best_all", trend = "local_level_0.01", autoAR = TRUE, seasonality = TRUE,
  holiday = TRUE, niter = n_iters)
model_483_impact <- CausalImpact(bsts.model = model_483$model,
                                 post.period.response = model_483$UnStandardize(as.numeric(model_483$post.period.response)),
                                 UnStandardize = model_483$UnStandardize)
plot(model_483_impact) # negative not significant

model_499 <- run_bsts_model(x, y, train_start, train_end, test_start, test_end, selected_controls,
  control_condidates = "best_all", trend = "static", autoAR = TRUE, seasonality = TRUE,
  holiday = TRUE, niter = n_iters)
model_499_impact <- CausalImpact(bsts.model = model_499$model,
                                 post.period.response = model_499$UnStandardize(as.numeric(model_499$post.period.response)),
                                 UnStandardize = model_499$UnStandardize)
plot(model_499_impact) # negative not significant

model_563 <- run_bsts_model(x, y, train_start, train_end, test_start, test_end, selected_controls,
  control_condidates = "best_all", trend = "local_level_0.01", autoAR = TRUE, seasonality = TRUE,
  holiday = FALSE, niter = n_iters)
model_563_impact <- CausalImpact(bsts.model = model_563$model,
                                 post.period.response = model_563$UnStandardize(as.numeric(model_563$post.period.response)),
                                 UnStandardize = model_563$UnStandardize)
plot(model_563_impact) # negative not significant
...
plot(impact) # negative impact
impact <- CausalImpact(train_data, c(train_start, train_end), c(test_start, test_end),
                       model.args = list(niter = n_iters))
plot(impact) # negative impact
