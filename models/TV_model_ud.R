...
names(output) <- c('RMSE', 'RMSE_sd', 'MAPE', 'MAPE_sd', 'Rsquare', 'AbsEffect', 'AbsEffect_CI_width', 'AbsEffect_sd', 'contain_zero')
      ...
      # 4.944184e+04       2.735633e+04       8.467464e-02       5.482319e-02
      #      Rsquare          AbsEffect AbsEffect_CI_width       AbsEffect_sd
      # 9.529186e-01       3.083878e+04       1.444950e+05       3.664353e+04
      # contain_zero
      # 8.809524e-01
...
p <- plot(model_31_impact) +
  ...
  labs(title = "Impact of the TV campaign in 6 weeks on Hindi Wikipedia unique devices from all of India",
  subtitle = "Vertical dashed line represents the date of the campaign 27 May 2018")
ggsave("hiwiki_ud_tv_impact.png", plot = p, path = 'docs/figures', units = "in", dpi = 300, height = 6, width = 14)
summary(model_31_impact)
summary(model_31_impact, "report")
png('docs/figures/hiwiki_ud_tv_inclusion_prob.png', units = "in", res = 300, height = 8, width = 10)
...
sort(colMeans(model_31$model$coefficients[-(1:burn),] != 0))
     # United.States_hi.wikipedia              Nepal_en.wikipedia
     #                 0.97529753                      0.98629863

# average coefficient
apply(model_31$model$coefficients[-(1:burn),], 2, PositiveMean)
     # United.States_hi.wikipedia              Nepal_en.wikipedia
     #                 0.183998069                      0.185963960
